...
def user_login(request):
    ...
    if request.method == "POST":
        ...
        if login_form.is_valid():
            ...
            return redirect(request.GET.get("from", reverse("blog:all_blogs")))
...
def change_nickname(request):
    redirect_to = request.GET.get("from", reverse("blog:all_blogs"))
    if request.method == "POST":
        form = ChangeNickNameForm(request.POST, user=request.user)
        if form.is_valid():
            nicknname_new = form.cleaned_data["nickname_new"]
            profile, created = Profile.objects.get_or_create(user=request.user)
            profile.nickname = nicknname_new
            profile.save()
...
def bind_email(request):
    ...
    return render(request, 'account/bind_email.html', context)
...
def change_password(request):
    redirect_to = reverse("blog:all_blogs")
    if request.method == "POST":
